...
__all__ = ['HybridEncoder_upsample']  # 定义模块公开接口
...
@register()
class HybridEncoder_upsample(nn.Module):
    ...
    def __init__(self,
                 ...
                 version='dfine',  # 版本选择
                 fpn_pan_blocks=None , #跨阶段增强增强提取网络层
        ...
        for _ in range(len(in_channels) - 1, 0, -1):  # 从深层到浅层
            # 根据版本选择不同的上采样方式
            if version == 'deim' and upsample_blocks in {'DySample_UP', 'EUCB', 'MEUM'}:
        ...
        for _ in range(len(in_channels) - 1, 0, -1):  # 从深层到浅层
            ...
            elif version == 'deim' and fpn_pan_blocks in {'C3k2',}:
            ...
            elif version == 'deim' and fpn_pan_blocks in {'C2f', }:
                print("Ai缝合怪-带你冲顶会、顶刊--使用了fpn_pan_blocks=",fpn_pan_blocks)
                module_blocks = globals()[fpn_pan_blocks]  # 从全局变量中获取
                self.fpn_blocks.append(
                    module_blocks(
        ...
        for _ in range(len(in_channels) - 1):
            ...
            elif version == 'deim' and downsample_blocks in {'SPDConv','HWDown','WTFDown','DRFDown'}:
